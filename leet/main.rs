...
pub mod coins {
    ...
    pub fn coins(s: &str) -> T {
        // store maze and coin coords
        let mut end = START;
        let mut maze = HashSet::new();
        let mut coin_coords = Vec::new();
        for (i, line) in s
            .lines()
            .map(|line| line.trim())
            .filter(|line| !line.is_empty())
            .enumerate()
        {
            for (j, ch) in line.chars().enumerate() {
                ...
                if ch == END {
                    ...
                    coin_coords.push(coord);
                }
            }
        }

        let bfs = |a: Coord, b: Coord| bfs(&maze, a, b);
        let n_coins = coin_coords.len();

        // early return: 0 coins
        if coin_coords.is_empty() {
            return bfs(START, end);
        }

        // cache distances w/ early return
        // start -> coin, coin -> end, coin -> coin
        let start_to_coin = coin_coords
            .iter()
            .map(|&c| bfs(START, c))
            .filter(|&d| d != -1)
            .collect::<Vec<_>>();
        let end_to_coin = coin_coords
            .iter()
            .map(|&c| bfs(c, end))
            .filter(|&d| d != -1)
            .collect::<Vec<_>>();
        if start_to_coin.len() != n_coins || end_to_coin.len() != n_coins {
            return -1;
        }
        let mut coin_distances = vec![vec![-1; n_coins]; n_coins];
        for (i, &start) in coin_coords.iter().enumerate() {
            for (j, &end) in coin_coords.iter().enumerate() {
                coin_distances[i][j] = bfs(start, end);
                if coin_distances[i][j] == -1 {
                    return -1;
                }
            }
        }
        ...
        // early return: 1 coin
        if coin_coords.len() == 1 {
            return start_to_coin[0] + end_to_coin[0];
        }

        let mut best = T::MAX;
        let mut coins = Vec::from_iter(0..coin_coords.len());

        // full distance across: start -> coins -> end
        let path_len = |coins: &[usize], best: T| -> T {
            let mut d =
                start_to_coin[coins[0]] + end_to_coin[*coins.last().unwrap()];

            let mut windows = coins.windows(2);
            while let Some(&[a, b]) = windows.next() {
                d += coin_distances[a][b];
                if d > best {
                    break;
                }
            }
        ...
        let mut permute_c = vec![0; coin_coords.len()];
        let mut permute_i = 1;
        while permute_i < coins.len() {
            best = best.min(path_len(&coins, best));
            permute(&mut coins, &mut permute_c, &mut permute_i);
        }
    ...
    fn bfs(maze: &Maze, start: Coord, end: Coord) -> T {
        ...
        let mut layer = HashSet::from([start]);
        let mut path_len = 0;

        loop {
            let mut new_layer = HashSet::new();
            let mut adj_coords = layer
                .iter()
                .flat_map(|&coord| adj(coord))
                .filter(|coord| !layer.contains(coord))
                .filter(|coord| !history.contains(coord))
                .filter(|coord| maze.contains(coord))
                .peekable();
            if adj_coords.peek().is_none() {
                return -1;
            }
            path_len += 1;
            for coord in adj_coords {
                ...
                new_layer.insert(coord);
            }
            history.extend(layer);
            layer = new_layer;
        }
    }

    // Heap's algorithm: https://en.wikipedia.org/wiki/Heap%27s_algorithm
    fn permute(v: &mut [usize], c: &mut [usize], i: &mut usize) {
        while *i < v.len() && c[*i] >= *i {
            c[*i] = 0;
            *i += 1;
        }
